...
class CSpec {
	...
	/* Lexical States. */
	Hashtable m_states; /*
						 * Hashtable taking state indices (Integer) to state
						 * name (String).
						 */

	/* Regular Expression Macros. */
	Hashtable m_macros; /*
						 * Hashtable taking macro name (String) to corresponding
						 * char buffer that holds macro definition.
						 */

	/* NFA Machine. */
	CNfa m_nfa_start; /* Start state of NFA machine. */
	Vector m_nfa_states; /*
						 * Vector of states, with index corresponding to label.
						 */

	Vector m_state_rules[]; /*
							 * An array of Vectors of Integers. The ith Vector
							 * represents the lexical state with index i. The
							 * contents of the ith Vector are the indices of the
							 * NFA start states that can be matched while in the
							 * ith lexical state.
							 */
	...
	Hashtable m_dfa_sets; /*
						 * Hashtable taking set of NFA states to corresponding
						 * DFA state, if the latter exists.
						 */
...
class CEmit {
	 ...
	 * 
	 * if (CUtility.DEBUG) { CUtility.ASSERT(null != m_spec);
	...
	private void emit_states() throws java.io.IOException {
		...
		while (states.hasMoreElements()) {
			...
			m_outstream.println("\tprivate final int " + state + " = "
	...
	private void emit_driver() throws java.io.IOException {
		...
		/*
		 * m_outstream.println("\t\t\t\t"); m_outstream.println("\t\t\t");
		 * m_outstream.println("\t\t\t"); m_outstream.println("\t\t\t");
		 * m_outstream.println("\t\t\t"); m_outstream.println("\t\t}");
		 */
	}
	...
	private void emit_actions(String tabs) throws java.io.IOException {
		...
		if (CUtility.DEBUG) {
			CUtility
					.ASSERT(m_spec.m_accept_vector.size() == m_spec.m_anchor_array.length);
		}
					...
					for (c = 0; c < m_spec.m_dtrans_ncols; ++c) {
						...
						if (goto_first != goto_next
							...
							if (CUtility.DEBUG) {
								...
								CUtility
										.ASSERT((m_group.size() - 1) == m_ingroup[next.m_label]);
							}
		...
		for (istate = 0; nstates > istate; ++istate) {
			...
			while (null != (dfa = get_unmarked())) {
				...
				for (i = 0; i < m_spec.m_dtrans_ncols; ++i) {
					...
					if (CUtility.DEBUG) {
						CUtility
								.ASSERT((null == bunch.m_nfa_set && null == bunch.m_nfa_bit)
...
public class Main {
	...
	public static void main(String arg[]) throws java.io.IOException {
		...
		/*
		 * Note: For debuging, it may be helpful to remove the try/catch block
		 * and permit the Exception to propagate to the top level. This gives
		 * more information.
		 */
		try {
...
class CLexGen {
	...
	private Hashtable m_tokens; /*
								 ...
								 * corresponding lexical code for the internal
								 * lexical analyzer.
								 */
	private CSpec m_spec; /*
	 ...
	 * Function: CLexGen
	 **************************************************************/
	CLexGen(String filename) throws java.io.FileNotFoundException,
			java.io.IOException {
	...
	private void userCode() throws java.io.IOException {
		...
		while (true) {
			...
			m_outstream
					.print(new String(m_input.m_line, 0, m_input.m_line_read));
		}
	}
	...
	private char[] packCode(char start_dir[], char end_dir[], char prev_code[],
		...
		if (CUtility.DEBUG) {
			CUtility.ASSERT(INIT_CODE == specified || CLASS_CODE == specified
